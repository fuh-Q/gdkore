...
def is_staff():
    async def check(ctx: commands.Context):
        ...
        if ("administrator", True) in list(ctx.author.guild_permissions) or (
            "manage_guild",
            True,
        ) in list(ctx.author.guild_permissions):
            return True

        client: AutokickBot = ctx.bot

        if (
            client.get_guild(client.config["main_guild"]).get_role(
                client.config["staff_role"]
            )
            in ctx.author.roles
        ):
            return True
...
class Autokick(BattlerCog):
    ...
    async def autokick_task(self):
        try:
            while True:
                try:
                    main_guild: discord.Guild = self.client.get_guild(
                        self.client.config["main_guild"]
                    )
                    wait_interval: float = self.client.config["wait_interval"]
                    verified_role: discord.Role = main_guild.get_role(
                        self.client.config["verified_role"]
                    )
                    whitelisted_role: discord.Role = main_guild.get_role(
                        self.client.config["whitelisted_role"]
                    )
                except Exception:
    ...
    def convert_time(self, time: str) -> float:
        ...
        if not matches or len("".join(matches)) != len(
            re.sub(r"\s", "", time, flags=re.I)
        ):
            raise ValueError
    ...
    async def find_role(
        ...
        role = match.extractOne(
            search, [r for r in ctx.guild.roles if r.position != 0], score_cutoff=0.2
        )
        if not role:
            return
    ...
    async def _wait_interval(self, ctx: commands.Context, time_arg: str):
        try:
            ...
            return await ctx.reply(
                content="Couldn't convert your time. Valid units are [`w`, `d`, `h`, `m`, `s`]"
            )
            ...
            return await ctx.reply(
                content=f"Set the wait interval for autokicking to {time}"
            )
    ...
    async def _verified_role(self, ctx: commands.Context, role_search: str):
        ...
        if not role:
            ...
            self.client.config.update(update)
            return await ctx.reply(
                content=f"Set the verified role to {role.mention}",
