...
class Logic:
    ...
    def move_piece(
        ...
        old = self._get_slot(piece.x, piece.y)
        assert old
        old.occupant = None
        old.piece = None
        ...
        if jump_confirm:
            ...
            assert jumped and jumped.piece is not None
    ...
    def _evolve_piece(self, piece: Piece) -> None:
        piece.king = True
...
class Game(View):
    ...
    def __init__(
        ...
        for move in MOVEMENTS:
            setattr(self, move.lower(), directional_button(self, move.lower()))
            self.add_item(getattr(self, move.lower()))

        self.separator = Button(label="\u200b", disabled=True, row=1)
    ...
    @button(emoji=BotEmojis.QUIT_GAME, style=discord.ButtonStyle.danger)
    async def forfeit(self, interaction: Interaction, btn: Button):
