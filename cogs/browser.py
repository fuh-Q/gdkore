    ...
    def courses_to_pages(self, *, courses: List[Course]) -> None:
        ...
        while courses != []:
            ...
            for course in bundle:
                page.add_field(name=course["name"], value=f"[*`course id` —* `{course['id']}`]({course['alternateLink']})")
...
class ClassPicker(Select[CoursePages]):
    ...
    async def callback(self, interaction: Interaction) -> None:
        ...
        if from_cache := self.view.cache.get(self.values[0], None):
            ...
            assignments = None
            course = next(filter(lambda i: i["id"] == self.values[0], chain.from_iterable(self.view._courses)))

        menu = ClassHome(self.view, interaction, course, self.view._resource, assignments)
        menu.original_message = self.view.original_message

        e = Embed(
            title=course["name"], description=f"{course.get('descriptionHeading', '')}\n\n{course.get('description', '')}"
        )
...
class ClassHome(GoBack[CoursePages]):
    ...
    def run_google(self, service: Resource, nextPageToken: str | None = None):  # all of the google libs are sync
        if nextPageToken is not None:
            extras = {"pageToken": nextPageToken, "pageSize": 50}
        else:
            extras = {"pageSize": 10}
        ...
        return (
            service.courses()
            .courseWork()
            .list(courseId=self._course["id"], orderBy="updateTime desc, dueDate desc", **extras)
            .execute()
        )
    ...
    async def setup_webhook(self, interaction: Interaction, button: Button):
        if not interaction.guild or not isinstance(interaction.channel, discord.TextChannel):
            return await interaction.response.edit_message(
                embed=Embed(description="webhook channel must be a text channel in a guild"), view=GoBack(self._home)
            )
        ...
        if not view.choice:
            return await view.interaction.response.edit_message(
                embed=Embed(description="phew, dodged a bullet there"), view=GoBack(self._home)
            )
    ...
    async def view_attachments(self, interaction: Interaction, button: Button):
        ...
        if (assignments := self._assignments) is None:
            e = Embed(description=f"{BotEmojis.LOADING} fetching data...")
            ...
            try:
                ...
                e = Embed(
                    description="you can't view assignments from a course owned by your own account."
        ...
        menu: ClassMenu = await ClassMenu().async_init(
            self._home, self._interaction, self._course, assignments, self._resource
        )
...
class ClassMenu(BasePages, auto_defer=False):  # type: ignore
    ...
    async def async_init(
        ...
        self._children.insert(0, self._children.pop(self._children.index(self.return_home)))
    ...
    async def make_embed(self, assignment: CourseWork) -> Embed:
        ...
        page = (
            ...
            .set_author(
                name=f"{cap(self._course['name']):256}", icon_url=ICONS[assignment_response], url=assignment["alternateLink"]
            )
        )
        ...
        if points := assignment.get("maxPoints", None):
            page.add_field(name="obtainable points", value=points)
    ...
    async def interaction_check(self, interaction: Interaction, item: Item) -> bool:
        if (allowed := await super().interaction_check(interaction, item)) and item in (
    ...
    def get_content(assignment: CourseWork) -> str:
        ...
        if now_utc.timestamp() >= due.timestamp():
            ...
            return "\N{WARNING SIGN}" f" assignment due soon. ㅤㅤ [<t:{time_int}:R>] " "\N{WARNING SIGN}\n\u200b"

        return ""

    @property
    def edit_kwargs(self):
        kwargs = {"content": None, "view": self}
...
class Browser(commands.Cog):
    ...
    async def list_courses(self, interaction: Interaction):
        ...
        def run_google_service(credentials) -> Resource:  # all of the google libs are sync
            creds = Credentials.from_authorized_user_info(credentials, scopes=self.client.SCOPES)
        ...
        async for courses in course_chunks:
            ...
            for _ in range(last_embed_slots_remaining):
                ...
                menu.pages[-1].add_field(
                    name=course["name"], value=f"[*`course id` —* `{course['id']}`]({course['alternateLink']})"
                )
