    ...
    def check_loss(self, blocks: list["Block"]) -> bool:
        ...
        if not empty_spaces:
            ...
            for b in self.blocks:
                if (
                    (blok := self._get_block(b.x - 1, b.y)) is not None and blok.value == b.value
                    or (blok := self._get_block(b.x, b.y - 1)) is not None and blok.value == b.value
                    or (blok := self._get_block(b.x + 1, b.y)) is not None and blok.value == b.value
                    or (blok := self._get_block(b.x, b.y + 1)) is not None and blok.value == b.value
                ):
                    can_save = True
...
class GameView(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        await msg.reply(f"ok im guessing you just <a:peace:951323779756326912>'d out on me cuz you havent clicked anything for 2 minutes")
...
class TwentyFortyEight(commands.Cog):
    ...
    async def twentyfortyeight(
        ...
        grid_size: Option(
            int, "what size grid you want", choices=[
