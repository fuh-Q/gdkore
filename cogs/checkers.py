    ...
    class cls(ui.Button):
        ...
        async def callback(self, interaction: Interaction) -> None:
            ...
            self.view.game.move_piece(
                ...
                direction.upper(),
                jump_confirm = (
                    True if piece is self.view.game.jumping_piece
                    else False
                )
            )
...
class Game:
    def __init__(self, players: List[discord.User], view: GV) -> None:
        ...
        self.users = players
        self.players = [Player(players[i], i) 
    ...
    def verify_directions(self, piece: Piece, *, jump_only: bool = False) -> Dict[str, bool]:
        ...
        for direction in directions.keys():
            ...
            directions[direction] = (
                ...
                and (sl.occupant is None and not jump_only # no one occupying it
                or self.check_jump(piece, direction)) # or occupied but we can jump it
                else False
            )
...
class GameView(BaseGameView):
    ...
    def _generate_select_options(self) -> List[discord.SelectOption]:
        ...
        options: List[discord.SelectOption] = sorted([discord.SelectOption(
            label=f"{ALPHABET[p.x]}{p.y + 1}",
            emoji=(
                BotEmojis.CHECKERS_RED if p.owner.number == 0 and not p.king
                else BotEmojis.CHECKERS_RED_KING if p.owner.number == 0 and p.king
                else BotEmojis.CHECKERS_BLUE if p.owner.number == 1 and not p.king
          ...
          if p.owner is self.turn
          and p is not self.selected], key = lambda k: k.value[1],
                                       reverse = True \
                                                 if self.turn is self.game.opponent \
                                                 else False)
    ...
    async def update_ui(self, interaction: Interaction | None = None) -> None:
        ...
        if self.game.loser:
            ...
            for c in self.children:
                ...
                and c is not self.piece_selector:
                    c.disabled = True
