...
class Game:
    def __init__(self, players: List[discord.User]):
        ...
        self.type = (
            GameType.SINGLEPLAYER if len(players) == 1
            else GameType.MULTIPLAYER
        )
...
class GameView(discord.ui.View):
    ...
    async def on_turn_timeout(self) -> None:
        ...
        if len([
            s.occupant for s in self.game._get_column(self.hovering - 1) 
    ...
    async def update_board(
        ...
        if not self.game.winner:
            ...
            yellows = ["<:yellowleft:964765364259012608>", "<:yellowright:964765364212863059>", "<:yellowdrop:964771745653481532>"]
            counter = 0
...
class ConnectFour(commands.Cog):
    ...
    async def connect4(self, interaction: Interaction, opponent: discord.User):
        ...
        content = "".join([
            ...
            else "<:lineyellow:964755893063393361>" if view.turn.number == 1
            else "<:lineneutral:964754865626705950>",
        ...
        setattr(
            view,
            "original_message",
            await interaction.channel.fetch_message((await interaction.original_message()).id)
        )
