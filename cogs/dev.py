...
class Dev(BattlerCog):
    ...
    async def blacklist(self, ctx, user: discord.Member, *, reason: str = "None given"):
        await self.client.blacklists.update_one({"_id": user.id}, {"$set": {"_id": user.id}}, upsert=True)
    ...
    @commands.command(aliases=["gbl"], hidden=True, brief="Add a server to the blacklist")
    @commands.is_owner()
    async def guildblacklist(self, ctx, guild: discord.Guild, *, reason: str = "None given"):
    ...
    @commands.command(aliases=["wl"], hidden=True, brief="Remove a user from the blacklist")
    @commands.is_owner()
    async def whitelist(self, ctx, user: discord.User):
        ...
        if is_blacklisted:
            ...
            try:
                await user.send("Congrats, you've been unbanned. Don't pull shit like that again")
            except Exception:
    ...
    @commands.command(aliases=["gwl"], hidden=True, brief="Remove a server from the blacklist")
    @commands.is_owner()
    async def guildwhitelist(self, ctx, guild: int):
        is_guild_blacklisted = await self.client.guild_blacklists.find_one({"_id": guild})
