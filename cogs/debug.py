...
class Jishaku(*OPTIONAL_FEATURES, *STANDARD_FEATURES):
    ...
    async def jsk(self, ctx: commands.Context):
        ...
        cache_summary = (
            f"{len(self.bot.guilds)} guild(s) and {get_member_count(self.bot)} user(s)"
        )
            ...
            with self.submit(ctx):
                with ShellReader(argument.content) as reader:
                    ...
                    interface = PaginatorInterFace(
                        ctx.bot, paginator, owner=ctx.author, emoji=emotes
                    )
    ...
    async def jsk_source(self, ctx: commands.Context, *, command_name: str):
        ...
        if file_format and not JISHAKU_FORCE_PAGINATOR:
            ...
            paginator = jishaku.paginators.WrappedPaginator(
