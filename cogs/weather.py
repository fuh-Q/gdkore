...
async def current_time(data: RT, img: Image.Image, draw: ImageDraw.ImageDraw):
    ...
    draw.text(
        ...
        string, (255, 255, 255), NotGDKID.medium_text
    )
    ...
    draw.text(
        ...
        string, (255, 255, 255), NotGDKID.normal_text
    )
...
async def celsius_deg(data: RT, img: Image.Image, draw: ImageDraw.ImageDraw):
    ...
    draw.text(
        ...
        deg, (59, 200, 93), NotGDKID.thiccc_text
    )
    ...
    draw.text(
        ...
        feels_like, (255, 255, 255), NotGDKID.normal_text
    )

async def forecast(data: RT, img: Image.Image, draw: ImageDraw.ImageDraw, precise: float):
    tz = timezone(timedelta(hours=-4))
    futures: List[Dict[str, Any] | None] = [obj for obj in data["forecast"]["forecastday"][0]["hour"] if obj["time_epoch"] > precise]
    if len(futures) >= 3:
        futures = futures[:3]
    else: # spilling into the next day
        futures += [obj for obj in data["forecast"]["forecastday"][1]["hour"]][:3 - len(futures)]
    
    def maf(w_multi: int, h_multi: int) -> Tuple[int, int]:
        return (int((img.width - weather_icon.width) * w_multi), int((index + h_multi) * (img.height / (len(futures) + 1))))
    ...
    for index, obj in enumerate(futures):
        ...
        draw.text(
            maf(0.6, 0.55),
            text, (255, 255, 255), NotGDKID.normal_text
        )
        img.paste(
            weather_icon,
            maf(0.6, 0.75),
        ...
        draw.text(
            maf(0.75, 0.65),
            temperature, (59, 200, 93), NotGDKID.medium_text
        )
        weather_icon.close()
...
async def weather_task(client: NotGDKID):
    ...
    if hasattr(client, "weather_hook"):
        await client.weather_hook.edit_message(client.weather_hook_msg_id, attachments=[file])
    else:
